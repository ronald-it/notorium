...
export const formatDates = (initialData: Notes): Notes => {
  const dataWithFormattedDate = {
    notes: initialData.notes.map((note: Note) => {
...
export const formatTagStrings = (initialData: Notes): Notes => {
  const dataWithTagStrings = {
    notes: initialData.notes.map((note: Note) => {
      note.tagString = note.tags.join(', ');
      return note;
    }),
  };
  return dataWithTagStrings;
};
